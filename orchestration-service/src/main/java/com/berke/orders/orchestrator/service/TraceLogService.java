...
public class TraceLogService {
    ...
    public void log(Long operationId, String iface, String dir, String status, Object req, Object res, String err) {
        ...
        traceRepo.save(OperationTraceEvent.builder().operationId(operationId).traceEventId(tid).stepNo(step).description(iface + " " + dir).build());
        logRepo.save(InterfaceLog.builder().operationId(operationId).traceEventId(tid).stepNo(step).interfaceName(iface).direction(dir).status(status).requestPayload(json(req)).responsePayload(json(res)).errorMessage(err).build());
    }
