...
Customer messages and product `ADD` messages use envelope version `1`. Product `REMOVE` uses version `2`, and tariff
`CHANGE` uses version `3`:
...
accept versions `1`, `2`, and `3`; customer consumers accept version `1`. Missing fields, unexpected event types, and other
...
A version `3` `CHANGE` command carries `existingProductInstanceId`, a mandatory reason, and exactly one Catalog-resolved
replacement tariff item. Its result contains the replacement item, allowing Orchestration to create its runtime mapping
before finalizing the same order. The correlation ID remains unchanged across both sides of the switch.
